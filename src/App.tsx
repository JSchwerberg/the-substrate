...
function App() {
  ...
  const endSectorExpedition = useGameStore(state => state.endSectorExpedition)
  ...
  // Hide loading screen once sector is generated (only during expedition mode)
  useEffect(() => {
    if (gameScreen === 'expedition' && currentSector) {
      // Small delay for smooth transition
      const timer = setTimeout(() => setIsLoading(false), 300)
      return () => clearTimeout(timer)
    } else if (gameScreen !== 'expedition') {
      // Reset loading state when leaving expedition mode
      setIsLoading(true)
    }
  }, [currentSector, gameScreen])
  ...
  return (
    <>
      ...
      {/* Campaign Map Screen */}
      {gameScreen === 'campaign_map' && <CampaignMap />}
      ...
      {gameScreen === 'expedition' && (
        <>
          ...
          >
            ...
            {expeditionResult !== 'active' && (
              <ExpeditionRewards
                onNewExpedition={() => {
                  ...
                  // If in campaign mode, end the sector expedition and return to campaign map
                  if (campaign) {
                    endSectorExpedition(expeditionResult)
                  }
                  // In standalone mode, rewards are claimed but sector stays active for reset
                }}
              />
            )}
            ...
            >
              <span>
                <strong>CONTROLS:</strong> 1=Scout | 2=Purifier | SPACE=Pause/Resume | TAB=Cycle
                Units | CLICK=Move | R=New Sector
              </span>
